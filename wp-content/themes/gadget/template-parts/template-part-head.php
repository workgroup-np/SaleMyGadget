		...
		<?php if (get_theme_mod( 'sale_my_gadget_account', 1 ) == 1  || has_nav_menu( 'top_menu' ) || get_theme_mod( 'sale_my_gadget_socials', 0 ) == 1 ) { ?>
			<div class="container">
							...
							<nav id="site-navigation-top" class="navbar navbar-inverse" role="navigation" itemscope itemtype="http://schema.org/SiteNavigationElement">                       
								<div class="navbar-header row">
									<button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-2-collapse">
										<span class="sr-only"><?php esc_html_e( 'Toggle navigation', 'sale-my-gadget' ); ?></span>
										<span class="icon-bar"></span>
										<span class="icon-bar"></span>
										<span class="icon-bar"></span>
									</button>
									<div class="navbar-text visible-xs"><?php esc_html_e( 'Menu', 'sale-my-gadget' ); ?></div>
								</div>   
								<?php
								wp_nav_menu( array(
				...
				<div class="header-login text-right text-left-xs col-sm-6 no-gutter pull-right"> 					
						...
						<?php } ?>
					<?php if ( get_theme_mod( 'sale_my_gadget_socials', 0 ) == 1 ) : ?>
						<?php sale_my_gadget_social_links(); ?>
					<?php endif; ?>  
				</div>
			</div>
		<?php } ?>
		...
		<div class="container">
			...
			<?php if ( class_exists( 'WooCommerce' ) ) : ?>
				<div class="header-right col-md-8" >
					...
					<?php // Shopping Cart ?>
					<?php if ( function_exists( 'sale_my_gadget_header_cart' ) && ( get_theme_mod( 'cart-top-icon', 1 ) == 1 || get_theme_mod( 'wishlist-top-icon', 0 ) == 1 && function_exists( 'YITH_WCWL' ) ) ) {
						sale_my_gadget_header_cart();
