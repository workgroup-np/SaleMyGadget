...
/* 
 * This file contain code for gadget widget.
 * 
 * @package gadget
 */
...
<div class="tabs-content">
  <section role="tabpanel" aria-hidden="false" class="content active" id="panel2-1">
	...
	<?php 
		$gadget_popularposts = array( 
			'ignore_sticky_posts' => true,
			'showposts' => esc_attr(get_theme_mod('gadget_widget_range','5')),
			'orderby' => 'comment_count',
							);
		$the_query = new WP_Query( $gadget_popularposts );
		if ( $the_query->have_posts() ) :
		while ( $the_query->have_posts() ) : $the_query->the_post();
	?>

	<div class="sidebarwidget1">
		<?php if ( has_post_thumbnail() ) {the_post_thumbnail('gadget_themewidget');} else { ?>
  ...
  <section role="tabpanel" aria-hidden="true" class="content" id="panel2-2">
	<?php 
		$gadget_recentposts = array( 
			'ignore_sticky_posts' => true,
			'showposts' => esc_attr(get_theme_mod('gadget_widget_range','5')),
			'orderby' => 'date',
							);
		$the_query = new WP_Query( $gadget_recentposts );
		if ( $the_query->have_posts() ) :
		while ( $the_query->have_posts() ) : $the_query->the_post();
	?>
	...
	<div class="sidebarwidget1">
		...
		<?php } ?> 
		<a title="<?php the_title(); ?>" href="<?php the_permalink() ?>" rel="bookmark"><?php the_title(); ?></a>
                <br /><div class="widgetinfo"><?php gadget_posted_on(); ?></div>
		<div class="clear"></div>
		</div>			
		<?php endwhile;  endif;  wp_reset_postdata(); ?>
  </section>
