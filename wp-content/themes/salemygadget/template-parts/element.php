...
<?php
$getcateid2 = get_theme_mod('gadget_catechoose2'); 
 ?>
	...
	<!---Title of Index Elements-->
		...
		<a href="<?php echo esc_url( get_category_link( $getcateid2 ) ); ?>" class="viewposts" title="<?php echo get_cat_name($getcateid2); ?>"><?php _e('View all','gadget'); ?> <i class="fa fa-caret-right"></i></a>
	</div>
...
<?php if ( get_theme_mod('comment_number' ) !='1') { echo '<div class="comment"><p>'; printf( _nx( '1', '%1$s', get_comments_number(), 'comments title', 'gadget' ), number_format_i18n( get_comments_number() ) ); echo '<p></div>'; } if ( has_post_thumbnail() ) { ?>
	<a href="<?php esc_url(the_permalink());?>"><?php the_post_thumbnail('gadget_indeximagebig'); ?></a>
<?php } else { ?>
...
<a href="<?php the_permalink(); ?>"> <?php the_post_thumbnail('gadget_indeximage'); ?></a>
<?php } else { ?>
<a href="<?php the_permalink(); ?>"><img src="<?php echo esc_url(get_template_directory_uri() ); ?>/images/thumb.jpg" class="blog-post-img"></a>
<?php } ?>
